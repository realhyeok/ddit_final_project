...
var yourRequestTable = $("#yourRequest").kendoGrid({
	dataSource: {
        type: "json",
        transport: {
        	read: "/app/myWork/getYourRequest?userTo=realhyuk@ddit.com"
        },
        pageSize: 10,
        schema: {
            model: {
...
var yourStatusDropdown = yourRequestTable.find("#yourStatus-category").kendoDropDownList({
	dataSource: {
		type: "json",
        transport: {
        	read: "/app/myWork/getYourRequestSort?userTo=realhyuk@ddit.com"
        },
		schema: {
			model: {
				fields: {
...
var myRequestTable = $("#myRequest").kendoGrid({
	dataSource: {
        type: "json",
        transport: {
        	read: "/app/myWork/getMyRequest?userFrom=realhyuk@ddit.com"
        },
        pageSize: 10,
        schema: {
            model: {
...
var myStatusDropdown = myRequestTable.find("#myStatus-category").kendoDropDownList({
	dataSource: {
		type: "json",
        transport: {
        	read: "/app/myWork/getMyRequestSort?userFrom=realhyuk@ddit.com"
        },
		schema: {
			model: {
				fields: {
